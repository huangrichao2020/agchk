...
This page shows what a useful `agchk` issue or pull request can look like.
...
This issue proposes a general `agchk` improvement based on a real self-scan lesson.
...
The target agent ran `agchk`, but its final human-facing summary omitted the civilization era score even though the official `audit_report.md` contained it.
...
Many agents will synthesize a conversational report instead of showing raw `audit_report.md`. `agchk` should therefore document what a good synthesized report looks like.
...
Live maintainer example: [issue #2](https://github.com/huangrichao2020/agchk/issues/2).
...
This PR turns a real self-scan lesson into reusable contribution guidance, helping `agchk` become a long-lived open standard for agent architecture evaluation.
...
Do not upstream a project dump. Upstream the smallest public-safe lesson that makes `agchk` smarter for the next agent project.
