...
The contract is especially important for file writes, shell commands, package publishing, pull requests, deployments, cron jobs, and gateway actions.

## Scanner Implication

`agchk` treats Stateful Agent as an OS-level recovery primitive.

It flags projects that mention context replay, resumable runs, or interrupted turns but do not name environment state, side-effect logs, and idempotent recovery. A good agent runtime should recover from the physical state of the work, not only from what the conversation says happened.
