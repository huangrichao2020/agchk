...
`agchk` should not pretend that regex scanners understand every agent architecture.
...
2. `agchk` runs its built-in static scanners as a stable external baseline.
...
This turns `agchk` from a blunt scanner into an audit protocol.
...
The target agent can also lie to itself. That is why `agchk` still runs the external scan.

## Methodology

Ask the target agent to produce a small self-review JSON before running `agchk`.
...
# 2. Run agchk with the self-review attached.
agchk . \
  --profile personal \
...
```json
{
  ...
  "agent_name": "MyAgent",
  "summary": "I inspected my own source tree before agchk ran. My main loop lives in src/agent.py; tools are routed through src/tools/registry.py; memory is split into facts, skills, and impressions.",
  "claims": [
    {
      "title": "Provider implementations are intentional extension points",
      "evidence": "src/providers/openai.py",
      "recommendation": "If agchk flags this as hidden LLM usage, treat it as provider-aware false-positive evidence."
    }
  ],
  "risks": [
    {
...
```text
请先按 agchk 的 target-agent-self-review 方法论审计你自己的源码和工作区，生成 self_review.json。不要只说文件是否创建，要检查完整闭环：文件创建 -> 索引更新 -> 印象卡片 -> 锚点映射 -> 指针注册 -> 验收。然后运行 agchk . --profile personal --self-review self_review.json，最后把你的自检结论、agchk 静态扫描结论、二者冲突点和下一步优化计划合成一份报告。
```

## Rule

The self-review does not override `agchk` findings. It contextualizes them.

When the target agent and `agchk` disagree, the report should treat that disagreement as a useful audit object:
