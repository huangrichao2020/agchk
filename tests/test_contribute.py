...
from agchk import cli
from agchk.contribute import CommandError, publish_bundle_to_upstream
...
def test_contribute_prepare_creates_bundle_from_audit_results(tmp_path: Path) -> None:
    ...
    assert bundle["bundle_version"] == "agchk.contribution.v1"
    assert bundle["source"]["target_name"] == "/tmp/example-agent"
    assert bundle["suggested"]["upstream_repo"] == "huangrichao2020/agchk"
...
def test_publish_bundle_to_upstream_uses_fork_based_pr_flow(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_run_command(args: list[str], cwd: str | None = None) -> str:
        ...
        if args[:3] == ["gh", "pr", "create"]:
            return "https://github.com/huangrichao2020/agchk/pull/999\n"

        raise AssertionError(f"Unexpected command: {args}")

    monkeypatch.setattr("agchk.contribute.ensure_gh_available", lambda: None)
    ...
    assert pr_url == "https://github.com/huangrichao2020/agchk/pull/999"
