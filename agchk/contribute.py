...
BUNDLE_VERSION = "agchk.contribution.v1"
DEFAULT_UPSTREAM_REPO = "huangrichao2020/agchk"
DEFAULT_BUNDLE_ROOT = Path(".agchk") / "contributions"
...
def ensure_gh_available() -> None:
    ...
    if shutil.which("gh"):
        return
    raise RuntimeError("GitHub CLI `gh` is required for `agchk contribute pr`.")
...
def render_pr_body(bundle: dict[str, Any], *, owner_consent: bool, public_safe: bool) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- [{'x' if mark('Scanner Infrastructure') == 'x' else ' '}] Scanner Infrastructure",
            f"- [{'x' if mark('Performance') == 'x' else ' '}] Performance",
            f"- [{'x' if mark('Release / CI') == 'x' else ' '}] Release / CI",
            ...
            "## Risk / Compatibility",
            "",
            "No user-visible compatibility change is expected unless described in the proposed upstream change. Review false-positive and false-negative risk before merging.",
            "",
            "## Why This Generalizes",
            "",
            bundle["contribution"]["why_generalizes"],
...
def _ensure_fork_exists(
    ...
    *,
    ...
    try:
        command_runner(["gh", "repo", "view", f"{login}/agchk", "--json", "nameWithOwner"], None)
    except CommandError:
        command_runner(
...
def publish_bundle_to_upstream(
    ...
    *,
    ...
    """Open a fork-based PR to the upstream agchk repository."""
    ...
    if managed_workspace:
        temp_workspace = tempfile.TemporaryDirectory(prefix="agchk-contribute-")
    ...
    try:
        ...
        command_runner(["gh", "repo", "clone", f"{login}/agchk", str(clone_dir), "--", "--depth=1"], None)
